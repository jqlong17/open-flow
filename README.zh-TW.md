...
Open Flow 目前同時有兩種發布形態：

- **GitHub 開源版**：本倉庫持續演進的較完整版本，保留更多跨平台支援與實驗功能，例如會議模式、草稿工作流與更多設定項
- **Mac App Store 版**：面向商店審核與穩定交付的精簡版本，目前聚焦 **本地離線麥克風轉寫**，並內建離線模型

這兩個版本共享同一個專案與核心方向，但**不會追求完全一致的功能暴露面**。如果你在 GitHub 截圖、issue 或文件中看到某些功能，而商店版暫時沒有，通常是刻意的產品分層，而不是安裝異常。

商店版採付費下載，但專案依然開源：

- `Mac App Store` 的定價主要用來覆蓋 Apple Developer Program 會員、簽名、分發與持續維護等成本
- 原始碼仍完整保留在 GitHub
- 如果你不想購買商店版，仍然可以直接取得原始碼自行建置

以下截圖與功能說明，除非另有標註，主要對應 **GitHub 開源版 / 較完整版本**。
...
安裝完成後**目前視窗**即可執行 `open-flow.exe start --foreground`；新開的終端機也會自動辨識指令。首次執行會自動下載約 230MB 模型。Windows 預設熱鍵為 **右側 Win 鍵**（舊設定也會自動相容）；若需要，也可改成 **右側 Alt**。轉寫結果在剪貼簿，在任意輸入框按 **Ctrl+V** 貼上。

**Windows / Linux 遠端排障建議**

如果啟動、熱鍵、錄音或音訊裝置有問題，請先執行：
...
它會輸出目前可執行檔路徑、設定檔路徑、音訊輸入裝置列表，以及 `daemon.log` 的最近日誌。把這段輸出直接貼給維護者，遠端排查會快很多。也歡迎大家提交 Issue 或 PR 一起改善 Windows / Linux 體驗。

**Windows / Linux 模型下載兜底**

如果預設的 Hugging Face 下載逾時，Open Flow 現在會自動依序嘗試官方來源與 `https://hf-mirror.com`。如果你的網路環境特殊，也可以手動指定下載來源：
...
Windows 預設的量化模型目錄通常是 `%APPDATA%\openflow\open-flow\data\models\sensevoice-small`。
...
> **說明**：Windows 預設熱鍵為 **右側 Win 鍵**（舊設定會自動相容）；全域熱鍵（rdev）在某些應用中可能仍需要**以系統管理員身分執行**才能生效。若無效可改用 `transcribe` 指令做單次錄音轉寫。
...
如果你使用的是包含會議模式或系統音訊採集的版本，可能還需要額外授權 **螢幕錄製**。目前的 `Mac App Store` 精簡版通常只需要上面三項權限。
...
其中會明確說明：提交到本倉庫的內容，可被專案用於開源分發，也可用於包括 App Store 在內的付費分發。
