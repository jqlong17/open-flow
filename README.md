...
Open Flow 当前会同时存在两个发布形态：

- **GitHub 开源版**：主仓库持续演进的完整版本，保留更多实验能力和跨平台支持，包括 macOS、Windows、Linux，以及会议模式、草稿整理、更多设置项等
- **Mac App Store 版**：面向商店审核和稳定交付的精简版本，当前聚焦 **本地离线麦克风转写** 这一条主路径，默认内置离线模型，并收敛部分实验功能与非必要入口

这两个版本会共享同一个项目和核心能力，但**不会追求完全一致的功能暴露面**。如果你在 GitHub 文档、截图或 issue 中看到某些功能，而商店版暂时没有，这通常是有意的产品分层，而不是安装异常。

商店版采用付费下载，并不影响项目开源：

- `Mac App Store` 定价主要用于覆盖 Apple 开发者会员、签名、上架与持续维护等现实成本
- 仓库代码仍然保持开源，愿意自行构建的用户依然可以直接从 GitHub 获取源码并本地编译使用
- 如果你不想通过 App Store 购买，完全可以选择自行构建；如果你希望获得更省事的安装、签名与分发体验，也可以直接购买商店版

### 使用方式

以下截图和功能说明主要覆盖 **GitHub 开源版 / 完整版**。如果你使用的是 **Mac App Store 版**，界面和功能会更精简，重点聚焦在本地离线麦克风转写这一条主路径。
...
权限页会集中展示应用运行所需的系统授权状态，方便首次安装和排障时快速检查。若你使用的是包含会议模式的版本，还会看到与系统音频相关的授权项。
...
从托盘菜单点击 **「偏好设置…」** 可打开图形化设置窗口，无需手动修改配置文件即可完成常用设置。

![Open Flow 设置界面 - 录音模式与输入源](assets/settings-app-recording-mode.png)

| 分页 | 功能 |
|------|------|
| **General** | 热键（右 Command / 右 Option / Fn / F13 等）、触发方式（按一次开关 / 按住录音）、**录音模式**、麦克风输入源、简繁转换 |
| **Recognition** | 识别引擎选择、本地 / 云端识别切换、语言相关设置 |
| **Models** | 本地模型状态、下载 / 重新下载、模型目录打开入口，以及相关模型配置 |
| **Vocabulary** | 个人热词、纠错开关、纠错提示词、API Key、本地词表入口 |
| **Meetings** | 会议模式状态、会话落盘目录与最近一次会议记录 |
| **Permissions** | macOS 权限状态、「打开设置」跳转与授权恢复提示 |
| **Logs** | 热键测试、运行日志、模型下载输出；完整版本还会包含会议排障相关信息 |

设置页把原本分散的选项收拢到了同一个窗口里。现在录音相关配置也更直观：先选 **录音模式**，再在需要时选择 **麦克风输入源**。保存后会立即写入本地设置；权限项会显示当前是否已授权，模型页也可以直接打开模型所在文件夹。
...
Open Flow 需要以下权限才能完整工作。**首次启动后请依次在系统设置中开启**：
...
| **屏幕录制** | 隐私与安全性 → 屏幕录制 | 仅在使用会议模式 / 系统音频采集时需要 |

> **说明**：如果你使用的是当前的 `Mac App Store` 精简版，一般只需要麦克风、辅助功能和输入监控三项权限，不需要屏幕录制。
>
> **排查提示**：若不确定授权是否生效，可在托盘 **「偏好设置…」** 的权限页查看状态，或查看日志：
>
> ```bash
...
- **Vocabulary 词库页**：集中管理个人热词、纠错模型、API Key 与本地词表文件
- 可选 **BigModel 轻量纠错**：结合个人热词，对转写结果做二次修正，改善专有名词、产品名与项目代号的识别结果
- GitHub 开源版提供 **桌面音频 + 麦克风（会议）** 模式，可同时采集系统声音与麦克风输入
- GitHub 开源版提供 **持续分段会议转写**，适合长会议记录与会后复盘
- GitHub 开源版提供 **录音草稿 AI 转换面板**，可在草稿右侧直接做文本整理、字幕生成和 Markdown 整理
- **macOS**：托盘菜单「偏好设置…」可打开图形化设置界面，集中管理热键、词库、识别引擎、模型、权限与日志
...
```bash
# 一键安装 GitHub 开源版（Apple Silicon 预编译包，首次自动下载 ~230MB 模型）
...
**或下载 GitHub 版 .app**（双击即运行）：前往 [Releases](https://github.com/jqlong17/open-flow/releases) 页面下载最新的 macOS 安装包，解压后将 **Open Flow.app** 拖入「应用程序」。运行后点击菜单栏托盘图标，选择 **「偏好设置…」** 即可打开图形化设置界面。

如果你更希望直接安装签名完成、开箱即用的精简版本，也可以选择 `Mac App Store` 版；如果你不想购买商店版，依然可以从本仓库自行构建。
