...
> **安装提示（macOS）**：如果首次双击时被系统拦截，或提示“无法打开 / 无法安装”，请前往 **系统设置 → 隐私与安全性**，在页面底部找到与 Open Flow 相关的安全提示，点击 **仍要打开** 或允许打开，然后再返回「应用程序」重新启动一次。
...
> **安装提示（macOS）**：
> 若下载后的 App 被系统拦截，请前往 **系统设置 → 隐私与安全性**，在页面底部允许打开 Open Flow，然后重新启动应用。
>
> 完成安装后，若不确定权限是否生效，可在托盘 **「偏好设置…」 -> 「一般」** 页面查看实时权限状态（绿色勾表示已授权）。
...
**macOS 本地上传 Release 安装包**：`./scripts/release.sh` → 使用本机 **Developer ID** 构建发布版 `.app`，并要求设置 `OPEN_FLOW_NOTARY_PROFILE` 完成公证后再上传到对应 GitHub Release

首次配置 notarization 凭据：

```bash
./scripts/setup-notary-profile.sh
```

脚本会引导你配置 `notarytool` profile，完成后即可执行：
...
```bash
cd settings-app
OPENFLOW_PERF_DEV_UI=1 swift build -c release
```
...
- `OPENFLOW_PERF_DEV_UI=1` 时，设置页 `Diagnostics` 中会显示 **Performance Logging** 开发模块
